...
public:
	...
	static void createInstance(const char* filename, int num_of_stickers, std::string D2units);
	static DeepBreathLog& getInstance();

	std::ofstream log_file;

protected:

	DeepBreathLog(const char* filename, int num_of_stickers, std::string D2units);

	static DeepBreathLog* _log;
